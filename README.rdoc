WORKING ON: * As a user, I want the links with the most votes to be listed first, so that I can know what's most popular on index. (writing a popular scope)

voting doesn't work perfectly(can't vote a second time)

Why is logging in changing the point values? I AM SO CONFUSED.

*add New page (with all new links). organized by time

*add images for flash[:notice] and flash[:alert]
* sum_total == link.votes.length
* image does not show when users are logged out
...
##Installation
Clone the repo in your terminal by entering:
