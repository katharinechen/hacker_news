WORKING ON: add bootstrap to pages

-add create to LinksController
-print it nicely on all.



add gem Mechanize (to get title)
...
* navbar home(with all articles), new(new articles), and submit(submit articles)
...
* As a user, I want to comment on a comment, so that I can have a discussion about the comment. This is hard!!! Don't feel like you have to complete this problem. It took me several hours the first time I did it.

* Here are a couple hints. Use polymorphism so that a comment can belong to a post or to another comment. Your associations will look something like this:
...
* Add user avatar and profile page

*   resources :links, :except => [:edit, :update] do
    resources :comments
  end

  * many :execpt resources links
...
* Why doesn't the session controller need a session object?
